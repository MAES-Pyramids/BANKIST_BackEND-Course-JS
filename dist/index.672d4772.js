...
function calculateUserName(accounts) {
    accounts.forEach((user)=>{
        user.username = user.owner.toLowerCase().split(" ").map((word)=>word[0]).join("");
    });
}
...
function displayMovements(account, sorted = false) {
    ...
    let sortedData;
    if (sorted == true) sortedData = account.movements.slice().sort((a, b)=>a - b);
    else sortedData = account.movements;
    ...
    <div class="movements__value">${formatNumbers(mov, currentAccount?.locale, currentAccount?.currency)}</div>`;
...
function calcDisplayBalance(account) {
    ...
    labelBalance.textContent = `${formatNumbers(account?.balance, account?.locale, account?.currency)} 💷`;
}
//-------------------------------------
function calcDisplaySummary(account) {
    const incomes = account.movements.filter((value)=>value >= 0).reduce((acc, value)=>acc + value, 0);
    labelSumIn.textContent = `${formatNumbers(incomes, account?.locale, account?.currency)}E£`;
    //----
    const outcome = account.movements.filter((value)=>value <= 0).reduce((acc, value)=>acc + value, 0);
    labelSumOut.textContent = `${formatNumbers(Math.abs(outcome), account?.locale, account?.currency)}E£`;
    //----
    const interest = account.movements.filter((value)=>{
        return value > 0 && account.interestRate * value / 100 >= 1;
    }).map((value)=>value * account.interestRate / 100).reduce((sum, value)=>sum + value, 0);
    labelSumInterest.textContent = `${formatNumbers(interest, account?.locale, account?.currency)}`;
}
//-------------------------------------
// login
let currentAccount;
...
btnLogin.addEventListener("click", function(def) {
    ...
    if (currentAccount?.pin === Number(inputLoginPin.value)) {
        ...
        // welcome
        labelWelcome.textContent = DefaultMessage = `Welcome Back Mr: ${currentAccount.owner.split(" ")[0]} 🥷`;
        // display
...
btnClose.addEventListener("click", (def)=>{
    def.preventDefault();
    const user = accounts.find((account)=>account.username === inputCloseUsername.value);
...
btnLoan.addEventListener("click", (def)=>{
    ...
    if (loan > 0 && currentAccount.movements.some((movement)=>movement >= loan * 0.1)) {
        // get loan
